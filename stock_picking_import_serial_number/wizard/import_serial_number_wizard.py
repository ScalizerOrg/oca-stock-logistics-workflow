...
class StockPickingImportSerialNumber(models.TransientModel):
    ...
    overwrite_serial = fields.Boolean(default=True)
    ...
    def _import_serial_number(self, xl_sheet, stock_move_lines, picking, file_extension):
        ...
        if file_extension == "xlsx":
            ...
            for row in rows:
                ...
                try:
                    package_value = row[self.sn_package_column_index] if len(
                        row) > self.sn_package_column_index else None
            ...
            for row_idx in range(1, xl_sheet.nrows):  # Skip header row
                ...
                serial_value = xl_sheet.cell_value(row_idx,
                                                   self.sn_serial_column_index) if xl_sheet.ncols > self.sn_serial_column_index else None
